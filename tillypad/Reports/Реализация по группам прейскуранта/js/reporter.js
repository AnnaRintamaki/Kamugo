...
// Здесь надо заполнить таблицу     
  ...
  $.get('Data/Details.xml', function(data) {
    ...
    $(data).find('ROW').each(function() {             
// Сумма реализации          
      var sumr = $(this).find('Sale_Sum').text();      
      if (parseFloat(sumr)=='NaN') sumr = '0';
      sumr = numeral(sumr).format($(this).find('CurrencyFormat').text().replace('руб','$'));
// Сумма по меню
      var summ = $(this).find('Mitm_Sum').text();      
      if (parseFloat(summ)=='NaN') summ = '0';
      summ = numeral(summ).format($(this).find('CurrencyFormat').text().replace('руб','$'));
// Сумма скидки
      var suml = $(this).find('Discount_Sum').text();      
      if (parseFloat(suml)=='NaN') suml = '0';
      suml = numeral(suml).format($(this).find('CurrencyFormat').text().replace('руб','$'));
// Сумма наценки
      var sumh = $(this).find('Margin_Sum').text();      
      if (parseFloat(sumh)=='NaN') sumh = '0';
      sumh = numeral(sumh).format($(this).find('CurrencyFormat').text().replace('руб','$'));
// Запись сумм в таблицу      
      $('#tapreport-table').append('<tr><td><p>'+$(this).find('mgrp_Name').text()+'</p></td><td align="right"><p>'+sumr+'</p></td><td align="right"><p>'+summ+'</p></td><td align="right"><p>'+suml+'</p></td><td align="right"><p>'+sumh+'</p></td></tr>');      
// Здесь надо заполнить таблицу
      all_data.push(parseFloat($(this).find('Sale_Sum').text())/total*100);                  
      all_ticks.push('<p text-align="right" vertical-align="top">'+$(this).find('mgrp_Name').text().replace(' ','\xA0')+'\xA0'+'\xA0'+'</p>');
      ++i;
    }); 
